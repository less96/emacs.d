...
Following contents are the original readme
* Features
- Efficient. Emacs *starts up in one second*
- Robust. Emacs never crash after a full package upgrade
...
If you use hunspell, check [[http://blog.binchen.org/posts/what-s-the-best-spell-check-set-up-in-emacs.html][my article]]. Please note the hunspell executable cannot understand both unix format path and windows format path at the same time. You need figure out hunspell version you are using.
